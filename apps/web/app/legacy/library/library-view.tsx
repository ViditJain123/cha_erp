...
export default function LibraryView({ initialDocs }: { initialDocs: LibraryDocMeta[] }) {
  ...
  async function search() {
    setBusy('Searching…');
    const res = await fetch('/api/legacy/library/search', {
      method: 'POST',
  ...
  async function ingestNotification() {
    ...
    const res = await fetch('/api/legacy/library/ingest', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
    ...
    if (!res.ok) return alert(await res.text());
    setIngest({ url: '', label: '' });
    setDocs(((await (await fetch('/api/legacy/library')).json()) as { docs: LibraryDocMeta[] }).docs);
  }
