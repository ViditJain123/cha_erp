...
function TariffTab({ rows, onChanged }: { rows: TariffMaster[]; onChanged: () => Promise<void> }) {
  ...
  async function add() {
    setBusy(true);
    const res = await fetch('/api/legacy/masters/tariff', {
      method: 'POST',
...
function RatesTab({ tables, onChanged }: { tables: ExchangeRateMaster[]; onChanged: () => Promise<void> }) {
  ...
  async function parse() {
    setBusy('Parsing…');
    const res = await fetch('/api/legacy/masters/exchange-rates/parse', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
  ...
  async function save() {
    if (!parsed) return;
    setBusy('Saving…');
    const res = await fetch('/api/legacy/masters/exchange-rates', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
...
function NotificationsTab({ onChanged }: { onChanged: () => Promise<void> }) {
  ...
  async function upload(file: File) {
    ...
    const res = await fetch('/api/legacy/masters/notifications/parse', { method: 'POST', body: form });
    setBusy(null);
    if (!res.ok) return alert(await res.text());
...
function MemoryTab({ rows, onChanged }: { rows: ProductMemory[]; onChanged: () => Promise<void> }) {
  async function remove(m: ProductMemory) {
    await fetch('/api/legacy/masters/product-memory', {
      method: 'DELETE',
      headers: { 'Content-Type': 'application/json' },
