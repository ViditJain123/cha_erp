...
export default function JobView({ initialJob }: { initialJob: JobRecord }) {
  ...
  async function approve() {
    ...
    const fresh = (await (await fetch(`/api/jobs/${job.id}`)).json()) as JobRecord;
    setJob(fresh);
    window.open(`/api/jobs/${job.id}/pdf`, '_blank');
  }
      ...
      <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">
            Sources:{' '}
            {job.fileNames.map((f) => (
              <a key={f} className="mr-2 text-indigo-600 hover:underline" target="_blank" href={`/api/jobs/${job.id}/files/${encodeURIComponent(f)}`}>
                {f}
              </a>
            ))}
          </p>
        ...
        <div className="flex gap-2">
          {job.status === 'approved' && (
            <a href={`/api/jobs/${job.id}/pdf`} target="_blank" className="rounded-lg border border-slate-300 bg-white px-4 py-2 text-sm font-medium hover:bg-slate-50">
              View PDF
            </a>
          )}
          <button
